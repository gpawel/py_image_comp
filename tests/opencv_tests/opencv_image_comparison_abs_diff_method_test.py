...
class TestOpenCVAbsDiff(BaseTest):
    ...
    def test_abs_diff_identical_grayscale_images(self, get_identical_image_path):
    ...
    def test_abs_diff_small_changed_grayscale_images(self, get_same_shape_mages_with_small_change):
    ...
    def test_abs_diff_identical_coloured_images(self, get_identical_image_path):
        img1, img2 = get_identical_image_path
        ratio = self.compare_using_opencv_method(img1, img2, lambda comp: comp.absolute_difference_coloured())
        abs_diff, mean_diff = ratio
        print(f"abs_dif: {abs_diff}\nmean_diff: {mean_diff}")
        assert abs_diff == 0
        assert mean_diff == 0

    def test_abs_diff_small_changed_coloured_images(self, get_same_shape_mages_with_small_change):
        img1, img2 = get_same_shape_mages_with_small_change
        ratio = self.compare_using_opencv_method(img1, img2, lambda comp: comp.absolute_difference_coloured())
        abs_diff, mean_diff = ratio
        print(f"abs_dif: {abs_diff}\nmean_diff: {mean_diff}")
